...
const TutorCard = props => {
    ...
    const [isFavorite, setIsFavorite] = useState(props.tutor.isFavorite);
    const history = useHistory();

    const onFavorite = () => {
        // alert('501 not implemented - (.../frontend/components/TutorList/TutorCard.js)');
        axios.post(`${process.env.REACT_APP_API_URL}/favorites/${props.tutor._doc._id}`, {}, { headers: { Authorization: `Bearer ${ctx.token}` } })
            .then(response => {
    ...
    useEffect(() => {
        if (showModal) {
            setIsLoading(true);
            setSelected(false);
            axios.get(`${process.env.REACT_APP_API_URL}/tutors/${props.tutor._doc._id}`, { headers: { Authorization: `Bearer ${ctx.token}` } })
                .then(response => {
    ...
    }, [showModal, props.tutor._doc._id, ctx.token])
    ...
    return (
        <React.Fragment>
                ...
                {confirmation && <div className='row text-center'>
                    ...
                    <div className='row mt-3'>
                        <div className='col-md-6'>
                            <button className='btn btn-primary col-12' onClick={() => { history.push('/profile') }}>My Profile</button>
                        </div>
                        <div className='col-md-6'>
                ...
                {!confirmation && <div>
                    <div className='row'>
                        <div className='col-md-12'>
                            <h5>Available appointments for {props.tutor._doc.firstName} {props.tutor._doc.lastName} for the next 7 days</h5>
                        </div>
                    </div>
                    <div className='row'>
            ...
            <div className="card shadow" style={{ height: '100%' }}>
                <img className="card-img-top" src={props.tutor._doc.profilePictureUrl} alt={`${props.tutor._doc.firstName} ${props.tutor._doc.lastName}`}></img>
                <div className="card-body d-flex flex-column justify-content-between">
                    <div className="d-flex flex-column mb-2">
                        <div className="d-flex justify-content-between align-items-center">
                            <h5 className="card-title">{props.tutor._doc.firstName} {props.tutor._doc.lastName}</h5>
                        ...
                        <span className="text-muted mb-1">{props.tutor._doc.aboutMe}</span>
                        <span className="text-muted mb-1">
                            <strong>Skills:</strong> {props.tutor._doc.skills.join(', ')}
                        </span>
                    </div>
